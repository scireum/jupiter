...
pub struct ConnectionInfo {
    ...
    /// Contains the number and average runtime of all commands issued via this
    /// connection.
...
/// Represents a RESP server which manages all TCP connections.
pub struct Server {
    running: AtomicBool,
            ...
            .map(|config| {
                ...
                format!(
                    ...
                    handle
                        .query("server.port")
                        .as_int()
                        .filter(|port| port > &0 && port <= &(std::u16::MAX as i64))
                        .unwrap_or(2410)
                )
            })
            ...
            match tokio::time::timeout(READ_WAIT_TIMEOUT, reader.read_buf(&mut input_buffer)).await
            {
                ...
                Ok(Ok(bytes_read)) if bytes_read > 0 => match Request::parse(&mut input_buffer) {
                    ...
                    Ok(Some(request)) => {
                        ...
                        // it here again...
                        if input_buffer.is_empty() && input_buffer.capacity() > DEFAULT_BUFFER_SIZE
                        {
